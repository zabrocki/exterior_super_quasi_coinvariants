...
def split(A):
    """
    Split the bi-exponent into pieces as needed in the computation of G(A)
    """
    k = last_zz(A)
    if k==0:
        U = [[],[]]
...
def list_bicomps_n_k_ell(n,k,ell):
    r"""
    list all bicompositions with top row sum n, bottom row sum k and length ell
    """
    if ell<max(n,k):
        return []
    if n==0:
...
def list_bicomps_m_ell(m,ell):
    """
    list all bicompositions whose total degree is m and whose length is ell
    """
    return sum((list_bicomps_n_k_ell(m-r,r,ell) for r in range(m+1)),[])

def list_bicomps(ell):
    """
    list all bicompositions whose length is ell and total degree <= ell
    """
    return sum((list_bicomps_m_ell(m,ell) for m in range(ell+1)),[])

def super_ideal(n):
    """
    the ideal in BR(n) generated by F(A) where A is a
    bicomposition whose length is ell and total degree <= ell
    """
    return BR(n).ideal([F(A) for A in list_bicomps(n) if sum(A[0])+sum(A[1])>0])

def list_exps_n_k_ell(n,k,ell):
    """
    list all the bi-exponents whose top sum is n, bottom sum is k and
    length ell
    """
