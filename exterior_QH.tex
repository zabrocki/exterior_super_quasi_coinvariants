...
$$\big\{ G_\alpha : \alpha \text{ \it crosses the diagonal}\big\}\subseteq I_n\,.$$
Furthermore, using Proposition~\ref{prop:leading}, it is linearly independent. 

We only need to show that
it $\mathbb Q$--spans the ideal. Usin Lemma~\ref{lem:spanofpolynomials} we have that
\begin{equation}\label{eq:idealpres}
 I_n = {\mathbb Q}\text{-span} 
\end{equation}
...
\nantel{we do not need what follow yet}
Let $u \in \{ 0, 1\}^{k-1}$ and $s>0$, then we
