...
We can therefore assume that $0<r\le \lfloor\frac{n}{2}\rfloor$  and $\theta^\beta \in B_n$
such that $m=m_1(\beta)\le \lfloor\frac{n}{2}\rfloor-r$.  We have
...
The idea is to start with $F_{1^r}\in X_n$ since $F_{1^r}=G_{1^r0^{n-r}}$ and then show that
the succession of multiplications by $\theta_{n-i+1}^{\beta_{i}}$ in the
order of Equation~\eqref{eq:thetaF} will preserve the property of being in $X_n$.
...
This ensures that every times we can use Equation~\eqref{eq:thetaG} on an element of $X_n$,
that the terms on the right hand side will also be in $X_n$.
...
\end{enumerate}
then $\theta_kG_\gamma\in X_n$. Note that we have made the substitution $s\rightarrow s+1$
...
If $k_1\le r$, then $r+2\le 2(r+m)\le 2\lfloor\frac{n}{2}\rfloor\le n$
