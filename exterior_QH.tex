...
$\alpha$ stays above the diagonal if $\sum_{i=1}^r a_i \leq \lfloor r/2 \rfloor$ 
\nantel{do we really need the floor here? wouldn't it be true if we just say $\sum_{i=1}^r a_i \leq  \frac{r}{2} $}
...
$r>0$. If $r+m_1(\beta)>\frac{n}{2}$, then Lemma~\ref{lem:degree} gives us the desired results. We can therefore assume that $0<r\le \frac{n}{2}$  and $\theta^\beta \in B_n$ such that $m=m_1(\beta)\le \frac{n}{2}-r$. We have
...
$1<j_1< j_2\cdots <j_m\le n$. Recall that $\beta$ stays above the diagonal if $\sum_{i=1}^h \beta_i\le \frac{h}{2}$. For $h=j_\ell$, we have
 $\ell=\sum_{i=1}^{j_\ell} \beta_i\le \frac{j_\ell}{2}$, or simply $j_\ell\ge 2\ell$. 
For $\phi(\theta^\beta)=\theta_{n-j_1+1} \theta_{n-j_2+1}\cdots \theta_{n-j_m+1}=\theta_{k_m} \theta_{k_{m-1}}\cdots \theta_{k_1}$, we have
 ...
 $$
We start with $\gamma^{(0)}=1^r0^{n-r}$. If $k_1\le r$ then we obtain
$1^{r+1}0^{n-r-1}$ and 
