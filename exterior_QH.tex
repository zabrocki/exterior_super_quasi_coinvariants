...
$r>0$. If $r+m_1(\beta)>\frac{n}{2}$, then Lemma~\ref{lem:degree} gives us the desired results. We can therefore assume that $0<r\le \frac{n}{2}$  and $\theta^\beta \in B_n$ such that $m_1(\alpha)\le \frac{n}{2}-r$. We have
\begin{equation}\label{eq:thetaF}
	\phi(\theta^\beta) F_{1^r} =  \theta_n^{\beta_1} \big(\cdots  \theta_2^{\beta_{n-1}} (\theta_1^{\beta_n}F_{1^r})\big)\,.
\end{equation}
The reversal of order in Equation~\eqref{eq:thetaF}, will be crucial in the proof.
The idea is to start with $F_{1^r}\in X_n$ since $F_{1^r}=G_{1^r0^{n-r}}$ and then show that the succession of multiplication by $\theta_i^{\beta_{n-i+1}}$ in the
