...
such that $m=m_1(\beta)\le \frac{n}{2}-r$.  We have
...
The idea is to start with $F_{1^r}\in {\mathbb Q}A_n$ since $F_{1^r}=G_{1^r0^{n-r}}$ and then show that
the succession of multiplications by $\theta_{n-i+1}^{\beta_{i}}$ in the
order of Equation~\eqref{eq:thetaF} will preserve the property of being in ${\mathbb Q}A_n$.

For this let  $u \in \{ 0, 1\}^{k-1}$ and $s>0$, then we
...
This ensures that every times we can use Equation~\eqref{eq:thetaG} on an element of ${\mathbb Q}A_n$,
that the terms on the right hand side will also be in ${\mathbb Q}A_n$.
Let us emphasize the requirement we need that allows us to use Equation~\eqref{eq:thetaG}. Given $\theta_k$ and $G_\gamma$ where $\gamma$ crosses the diagonal; if
\begin{enumerate}
	\item[(a)] $\gamma_{k}\gamma_{k+1}\cdots\gamma_n = 1^{s}0^{n-k-s+1}$ for some $s\ge 0$,
	\item[(b)] the position of the rightmost $1$ is at $k+s+1\le n$,
\end{enumerate}
then $\theta_kG_\gamma\in {\mathbb Q}A_n$. Note that we have made the substitution $s\rightarrow s+1$
in Equation~\eqref{eq:thetaG} to get the two conditions (a) and (b).
The order we use in Equation~\eqref{eq:thetaF} will ensure that (a) is satisfied as long as (b) is satisfied. We thus need to show that (b) is satisfied throughout.
...
If $k_1\le r$, then $r+2\le 2(r+m)\le 2\big(\frac{n}{2}\big)\le n$
and the indices we obtain in Equation~\eqref{eq:thetaG} are either
$1^{r+1}0^{n-r-1}$ or $1^{k_1-1}01^{r-k_1+1}0^{n-r-2}$.
If $k_1>r$, then $k_1+1\le (n-2m+2-1)+1\le (n-1)+1=n$
and the indices we obtain are $1^{r}0^{k_1-r-1}10^{n-k_1}$ or $1^{r}0^{k_1-r-1}010^{n-k_1-1}$.
In all cases we have that the entries in positions $k_1+1,k_1+2,\ldots, n$ are of the form
$1^a0^{n-k_1-a}$ for some $a> 0$ and the position of the last $1$ is at most $\max(r+2,k_1+1)\le n$.
...
\begin{align}
 r+2j&\le  2(r+m)\le 2\big({\textstyle \frac{n}{2}}\big)\le n \label{eq:rplus}\\
...
the position of the rightmost 1 will be at most $\max\big( r+2m, k_i+2(m-i)+1\big)_{1\le i\le m}\le n$.
