...
The argument for the proposition is elementary (chasing the leading term in \eqref{eq:0start} and \eqref{eq:0start})
...
The proof of this theorem will require a few auxiliary results and will be done in Section~\ref{ss:proofmainthm}.
...
tells us  that $G_\gamma \in I_n$. Proposition~\ref{prop:leading} say that $G_\gamma = \theta^\gamma + \sum_{\beta<\gamma}  c_\beta \theta^\beta$. Hence, modulo $I_n$, we have
 $$ \theta^\gamma \equiv \theta^\gamma - G_\gamma = - \sum_{\beta<\gamma} c_\beta \theta^\beta\,.$$
 The right hand side is a linear combination of monomials strictly smaller than $\theta^\gamma$. By choice of $\theta^\gamma$, all such monomial are in the $\mathbb Q$--span of $B_n$.
...
We proceed by induction of the degree $d=m_1(\gamma)$. If $d=0$, then $\theta^\gamma = 1 = 1\cdot F_{1^0} \in {\mathbf B}_n$.
...
The difference of both side is in the ideal $I_n$ and we can write
\begin{equation}\label{eq:expansioninB}
 \theta^\gamma = \sum_{\theta^\beta\in B_n}  c_\beta \theta^\beta + \sum_{\delta, r>0} d_{\delta,r} \theta^\delta F_{1^r}
 \end{equation}
This must be an homogenious expresssion of degree $d$, and since $r>0$, we must have that $m_1(\delta)<d$. By induction hypothesis,
$\theta^\delta$ is in the $\mathbb Q$--span of ${\mathbf B}_n$ and we have
 $$\theta^\delta F_{1^r} = \sum_{\theta^\beta\in B_n, s\ge 0} b_{\beta,s} \theta^\beta F_{1^s} F_{1^r} =  \sum_{\theta^\beta\in B_n, s\ge 0} ab_{\beta,s} \theta^\beta F_{1^{s+r}} \,.$$
The equality $F_{1^s} F_{1^r}  = a F_{1^{s+r}}$ comes from the fact that quasisymmetric function in $R_n$ form an algebra. It is the pure exterior component of the super-quasysimmetric 
algebra defined in \cite{FLP}. this shows that the right hand side of~\eqref{eq:expansioninB} is in the $\mathbb Q$--span of ${\mathbf B}_n$ which conclude the proof.
\end{proof}
...
\begin{lemma}\label{lem:degree}
   Any $\theta^\gamma$ of degree $m_1(\gamma)>\frac{n}{2}$ is in  $\mathbb Q$-span$\big\{ G_\alpha : \alpha \text{ \it crosses the diagonal}\big\}$.
\end{lemma}

\begin{proof} The proof is very similar  to Lemma~\ref{lem:spanofquotient}.
Order the monomial of degree $>\frac{n}{2}$ lexicographically. 
Let  $\theta^\gamma$ be the smallest such monomial that is not in  $\mathbb Q$-span $\big\{ G_\alpha : \alpha \text{ \it crosses the diagonal}\big\}$.
We have  that $\gamma$ crosses the  diagonal somewhere, since $m_1(\gamma)>\frac{n}{2}$.
Proposition~\ref{prop:leading} say that $G_\gamma = \theta^\gamma + \sum_{\beta<\gamma}  c_\beta \theta^\beta$. Hence
 $$ \theta^\gamma = G_\gamma  - \sum_{\beta<\gamma} c_\beta \theta^\beta\,.$$
 The right hand side is a linear combination of monomials strictly smaller than $\theta^\gamma$. By choice of $\theta^\gamma$, all such monomial are in 
 $\mathbb Q$-span $\big\{ G_\alpha : \alpha \text{ \it crosses the diagonal}\big\}$. 
 Therefore so is $\theta^\gamma$, a contradiction. We must conclude there are no such $\theta^\gamma$ and the lemma follows.\end{proof}
...
Let $A_n=\phi(B_n)$ for $B_n$ given in Lemma~\ref{lem:spanofquotient}.
\begin{equation}\label{eq:autoquasi}
I_n = {\mathbb Q}\text{\rm -span}\big\{ \theta^\alpha F_{1^r} : \theta^\alpha \in A_n,\  r>0\big\}
\end{equation}
\end{lemma}
...
\begin{align*}
 ...
 \end{align*}
For $\theta^\beta \in B_n$,  we have $\phi\big(\theta^\beta F_{1^r}\big)=\phi(\theta^\beta) \phi\big(F_{1^r}\big) = \pm \theta^\alpha F_{1^r}$ for $\theta^\alpha\in A_n$, and the lemma holds.
\end{proof}
...
$$X_n \subseteq I_n\,.$$
Furthermore, using Proposition~\ref{prop:leading}, we know that $\{ G_\alpha : \alpha \text{ \it crosses the diagonal}\big\}$ is linearly independent. 
We must now show that $I_n\subseteq X_n$. Using Lemma~\ref{lem:idealpres} it is sufficient to show that $\theta^\alpha F_{1^r}\in X_n$ for any $ \theta^\alpha \in A_n$ and
$r>0$. If $r+m_1(\alpha)>\frac{n}{2}$, then Lemma~\ref{lem:degree} gives us the desired results. We can therefore assume that $0<r\le \frac{n}{2}$  and $\theta^\alpha \in A_n$ such that $m_1(\alpha)\le \frac{n}{2}-r$. We have
\begin{equation}\label{eq:thetaF}
	\theta^\alpha F_{1^r} = \pm \theta_n^{\alpha_n} \big(\cdots  \theta_2^{\alpha_2} (\theta_1^{\alpha_1}F_{1^r})\big)\,.
\end{equation}
The reversal of order in Equation~\eqref{eq:thetaF} and the use of $A_n$ instead of $B_n$, will be crucial later on in the proof.
The idea is to start with $F_{1^r}\in X_n$ since $F_{1^r}=G_{1^r0^{n-r}}$ and then show that the succession of multiplication by $\theta_i^{\alpha_i}$ in the
...
that if $n-k-s-1 \geq 0$ we have
\begin{equation}\label{eq:thetaG}
    \theta_k G_{u1^{s-1}0^{n-k-s+2}} = (-1)^{m_1(u)}( G_{u1^{s}0^{n-k-s+1}} - G_{u01^{s}0^{n-k-s}} )\,.
\end{equation}
Moreover if $u1^s0^{n-k-s+2}$ crosses the diagonal then both
$u1^{s}$ and $u01^{s}$ cross the diagonal. This ensure  that every times we can use Equation~\ref{eq:thetaG} on an element on $X_n$, we remain in $X_n$.
Let us emphasis the requirement for Equation~\ref{eq:thetaG}. 




[...]\nantel{More details to be filled in later}
