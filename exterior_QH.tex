...
\end{align*}
To finish the  proof of Equation~\eqref{eq:0start} by induction, lets assume that $0\alpha\ne 01^s0^{n-s-1}$. Therefore $0\alpha=0w01^s0^{n-k-s}$ for some string $w$
of length $k-2$. For $0\alpha=0w01^s0^{n-k-s}$,  we have $n-k-s$ trailing zeros. Remark that for $0w1^s0^{n-k-s+1}$
and $0w1^{s-1}0^{n-k-s+2}$ we have more trailing zeros than $0\alpha$ and we will  use the Induction Hypothesis (with~\eqref{eq:0start})  in the
...
$G_{w01^s0^{n-k-s+1}} \in R_{n-1}$ and the equation~\eqref{eq:final0} follows. This conclude the proof of \eqref{eq:0start}.
...
\begin{align}
G_{1^{s+1}0^{n-s-1}} &= F_{1^{s+1}}(\theta_1, \theta_3, \ldots, \theta_n) \nonumber \\
...
&= \theta_1 G_{01^s0^{n-s-1}} + P_{1^{s}0^{n-s-1}}(\theta_2, \theta_3, \ldots, \theta_n)~. \nonumber
\end{align}
...
The term with $\theta_1$ is the definition of $G_{0\alpha}$.
The expression inside the square bracket is a polynomial in $R_{n-1}$ that we take as the definition for $P_\alpha$.
This shows that \eqref{eq:1start} holds in all cases and conclude the  proof of the lemma.
\end{proof}
