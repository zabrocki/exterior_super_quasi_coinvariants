...
If $\Phi(A)=A$, we have that $|Inv(A)|$ is even (we can match the pair two by two. Therefore $\Phi$ is a sign reversing involution and all fixed points contribute in Equation~\eqref{eq:signa} with a $+1$. Therefore
...
This can be derived using standard techniques of generating functions. \kelvin{I always think a family of coefficients should come with a GF for the sake of easy comparison. We should also add this to OEIS at some point.}
...
EQC_n \simeq EQH_n~.\]
\end{prop}
\nantel{I think I found the solution to $EQH_n$ and its (non)-relation to temperley-Lieb  representation. I will 
start writting and hopefully it is all right...if I am, it will simplify the proof of Section 4 by a lot!!! }
...
list $(C_1, C_2, \ldots, C_k)$ with $C_r = (i_r, j_r) \in [n] \times [n]$
\begin{align*}
...
Let 
 $$\Delta_C=(\theta_{j_1}-\theta_{i_1})(\theta_{j_2}-\theta_{i_2})\ldots (\theta_{j_k}-\theta_{i_k}).$$
...
$ \partial_{\theta_i}\Delta_C=0$. For the last equality, fix $1\le r\le k$. We bring the term $(\theta_{j_r}-\theta_{i_r})$  in front  of $\Delta_C$ performing $r-1$ anticommutation. That is $\Delta_C =(-1)^{r-1}(\theta_{j_r}-\theta_{i_r}) q$ for some  polynomial $q$. Therefor
