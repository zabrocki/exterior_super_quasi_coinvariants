...
\begin{lemma}
 The set $B_n=\big\{ \theta^\beta : \beta \text{ \it stays above the diagonal}\big\}$ $\mathbb Q$--span the quotient $R_n\big/ I_n$.
\end{lemma}

\begin{proof}
Order the monomial lexicographically. Let  $\theta^\gamma$ be the smallest monomial that is not in the $\mathbb Q$--span of $B_n$
(modulo  $I_n$). We must have  that $\gamma$ crosses the  diagonal, since otherwise $\theta^\gamma\in  B_n$.
\end{proof}
...
\nantel{we need the reference of Lapointe et al. for showing that Anti-QSym is an algebra.}
