...
} \footnote{nantel: the operator exists... but it is zero :)}
...
\end{equation}
In commutative variables, similar polynomial were defined by  Aval-Bergeron-Bergeron~\cite{AB,ABB} 
as a (complete) subset of $S$-polynomials needed to compute all possible $S$-polynomial in the Buchburger algorithm 
for gr\"obner basis. It is not given that one can describe easily such set of $S$-polynomial and here we have to adapt the definition for the type of variables we have.
...
of $G_\alpha$ because the definition in Equation \eqref{eq:Gdef2}
was adapted so that this lemma holds.

\begin{lemma} Let $\alpha \in \{0,1\}^{n-1}$, then
\begin{align*}
G_{0\alpha} &= G_\alpha(\theta_2, \theta_3, \ldots, \theta_n)\\
\hbox{and }G_{1\alpha} &= \theta_1 G_{0\alpha} + P_{1\alpha}(\theta_2, \theta_3, \ldots, \theta_n)
\end{align*}
where $P_{1\alpha}(\theta_2, \theta_3, \ldots, \theta_n) \in R_n$ is a polynomial
in the variables $\theta_2, \theta_3, \ldots, \theta_n$.
\end{lemma}

\begin{proof}
The proof will be by induction (on what) after we establish
two initial base cases.  The first is if $\alpha = 0^n$, then
\[
G_{0^n} = F_{1^0} = 1 = G_{0^{n-1}}(\theta_2, \theta_3, \ldots, \theta_n)
\]
\end{proof}
...
\href{https://doi-org/10.1006/aima.1999.1901}.
