...
EQC_n \simeq EQH_n~.
\]
\mike{todo: make that clear that it is
a basic property of vector spaces that
this holds true}
\end{prop}
...
make sure it is used properly.  Definition 3 in Chapter 1 of Cox, Little, O'Shea
\cite{CLO}
$x^\alpha <_{LEX} x^\beta$ if and only if $\alpha <_{LEX} \beta$.}
\kelvin{Done. As far as I can tell, we are using the above definition for comparison for
  the rest of the paper. We always explicitly mention lexicographical in subsequent
  use of the order. So I feel putting a subscript in the definition is a little redundant.}
