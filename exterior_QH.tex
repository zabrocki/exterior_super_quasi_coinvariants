...
\end{equation}
for some coefficients $d_{\delta,r}$.\mike{todo: $d$ is used up above, we should pick another letter for the coefficients}
...
for some coefficients $b_{\beta,s}$,
...
The equality $F_{1^s} F_{1^r}  = a F_{1^{s+r}}$\mike{todo: $a$ depends on $r$ and $s$} comes from the fact that
quasisymmetric functions in $R_n$ forms an algebra.\mike{todo: this should go in a remark in the section
...
$A_n:=\big\{ G_\alpha : \alpha \in \{0,1\}^n \text{ \it crosses the diagonal}\big\}$.
\end{lemma}
